...
set.seed(123)
step_cox <- step(cox_model ,direction = "both",trace = FALSE) ### 逐步回归，采用前向/后向/双向的规则去除冗余或无法使预测效能更佳的因素。参数还有forward和both
...
cox_results <- data.frame(
  ...
  HR = cox_summary$coefficients[, "exp(coef)"],  # 提取 HR 值
  logHR = log(cox_summary$coefficients[, "exp(coef)"]),  # 计算 log(HR)
...
ggplot(cox_results, aes(x = reorder(Otu, logHR), y = logHR, fill = logHR > 0)) +
  # geom_bar(stat = "identity", width = 0.6) +  # 画条形图
  geom_col()+
  # geom_text(aes(label=label),check_overlap = TRUE,size= 4,hjust = ifelse(logHR > 0, 0, 1))+
  coord_flip() +  # 旋转，使基因名称更清晰
...
### 4. Stratify patients based on the abundance of significant microbes
core_features
  ...
  for (i in 1:length(core_features)){
    ...
    if (genus %in% colnames(in_matrix)){
      ...
      if(all(df_s[,genus]==0)){
        ...
        res.cut2 <- surv_cutpoint(df_meta_ACC_m, #数据集
                                  time = "PFS_Months",
                                  event = "PFS_Status",
                                  minprop = 0.01,
                                  variables = 'taxa') #需要计算的数据列名
        ...
        res.cox2 <- coxph(Surv(PFS_Months, PFS_Status) ~ taxa_abun, data = df_meta_ACC_m) #按照丰度做cox
        # res.cox2 <- coxph(Surv(PFS_Months, PFS_Status) ~ taxa, data = dat2) #按照最佳分组做cox
