...
fun_tme_score <- function(exp){
  ...
  # gsva
  gene_set<-read.csv("./source_data/gsva_immune_cells.csv")[, 1:2]#https://www.cell.com/cms/10.1016/j.celrep.2016.12.019/attachment/f353dac9-4bf5-4a52-bb9a-775e74d5e968/mmc3.xlsx
...
fun_heatmap_immune_score <- function(df_cluster_clinical_tme){
  ...
  p <- ComplexHeatmap::Heatmap(data_nor, 
                          ...
                          # column_order=colnames(data_nor)[c(grep("MS1",pd$genus_tcga_s_combined),grep("MS2",pd$genus_tcga_s_combined))],
                          row_split = pd2$immue_type,
                          row_gap = unit(8, "mm"),
                          # row_order = rownames(data_nor)[c(grep('EPIC',rownames(data_nor)),
                          #                                  grep('quantiseq',rownames(data_nor)),
                          #                                  grep('MCPcounter',rownames(data_nor)),
                          #                                  grep('customed',rownames(data_nor)))],
                          column_title = NULL,
...
ggplot(data,aes(x=MS,y=`CD56bright natural killer cell_customed`,fill=MS))+
  ...
  labs(x="", y = "CD56bright natural killer cell", fill = "")+  # fill为修改图例标题
...
ggplot(data,aes(x=MS,y=`Memory B cell_customed`,fill=MS))+
  ...
  scale_fill_manual(values = c(MS1="#0072B5CC",MS2="#E18727CC"))+
  guides(fill=guide_legend(direction = "horizontal",nrow =1,position = 'bottom'))+
  labs(x="", y = "Memory B cell", fill = "")+  # fill为修改图例标题
  stat_compare_means(method = "wilcox.test",label = "p.format",size=6)+
  thm
# dev.off()


### microbial cluster and immune inhibitory genes
...
significant_genes <- p_values %>%dplyr::filter(p < 0.05) %>% pull(Gene)  # 提取显著 Gene 的名字
