...
fun_matrix_transfer <- function(matrix,method=c('com','com_filter','clr','clr_filter')){
  ...
  if(method=='com'){
    ...
    matrix <- matrix %>% select_if(~ sum(.) > 0) 
    mrexp <- newMRexperiment(
      counts = matrix,  
      phenoData = NULL      
    )
    mrexp_css <- cumNorm(mrexp, p = 0.5)
    matrix_css <- MRcounts(mrexp_css, norm = TRUE, log = FALSE)
    matrix_css <- t(matrix_css)
    matrix_css_log2 <- log2(matrix_css + 1)
    matrix_transfer <- matrix_css_log2
  }else if(method=='css_filter'){
    
    matrix1 <- matrix %>% 
      select_if(~ sum(.) > 0) %>%
      microbiome::transform("compositional") %>%   #compositional要求输入otu*sample
      .[rowMeans(.> 0.0001) > 0.01,] 
    matrix2 <- matrix[rownames(matrix1),colnames(matrix1)]
...
# df3 <- read.xlsx('./00_data_preparation/otu_genus_count_table.xlsx',sheet = 3,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric))

# df3 <- read.xlsx('./00_data_preparation/otu_species_count_table.xlsx',sheet = 1,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric))
# df4 <- read.xlsx('./00_data_preparation/otu_species_count_table.xlsx',sheet = 2,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric))

# df1 <- read.xlsx('./00_data_preparation/otu_genus_cpm_table.xlsx',sheet = 1,rowNames = TRUE)  %>%  mutate(across(where(is.character), as.numeric))
# df2 <- read.xlsx('./00_data_preparation/otu_genus_cpm_table.xlsx',sheet = 2,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric)) 
# df3 <- read.xlsx('./00_data_preparation/otu_species_cpm_table.xlsx',sheet = 1,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric)) 
# df4 <- read.xlsx('./00_data_preparation/otu_species_cpm_table.xlsx',sheet = 2,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric))
...
# df_otu_species_tcga_z <- fun_matrix_transfer(df3,method = 'com');dim(df_otu_species_tcga_z)
# df_otu_species_tcga_s <- fun_matrix_transfer(df4,method = 'com');dim(df_otu_species_tcga_s)

# df_otu_genus_tcga_z <- fun_matrix_transfer(df1,method = 'com_filter');dim(df_otu_genus_tcga_z)
# df_otu_genus_tcga_s <- fun_matrix_transfer(df2,method = 'com_filter');dim(df_otu_genus_tcga_s)
# df_otu_species_tcga_z <- fun_matrix_transfer(df3,method = 'com_filter');dim(df_otu_species_tcga_z)
# df_otu_species_tcga_s <- fun_matrix_transfer(df4,method = 'com_filter');dim(df_otu_species_tcga_s)
...
# df_otu_genus_tcga_r <- fun_matrix_transfer(df3,method = 'clr');dim(df_otu_genus_tcga_r)

# df_otu_species_tcga_z <- fun_matrix_transfer(df3,method = 'clr');dim(df_otu_species_tcga_z)
# df_otu_species_tcga_s <- fun_matrix_transfer(df4,method = 'clr');dim(df_otu_species_tcga_s)  # otu*sample

# df_otu_genus_tcga_z <- fun_matrix_transfer(df1,method = 'clr_filter');dim(df_otu_genus_tcga_z)
# df_otu_genus_tcga_s <- fun_matrix_transfer(df2,method = 'clr_filter');dim(df_otu_genus_tcga_s)
# df_otu_genus_tcga_r <- fun_matrix_transfer(df3,method = 'clr_filter');dim(df_otu_genus_tcga_r)
# 
# df_otu_genus_tcga_z <- fun_matrix_transfer(df1,method = 'css_filter');dim(df_otu_genus_tcga_z)
# df_otu_genus_tcga_s <- fun_matrix_transfer(df2,method = 'css_filter');dim(df_otu_genus_tcga_s)
# df_otu_genus_tcga_r <- fun_matrix_transfer(df3,method = 'css_filter');dim(df_otu_genus_tcga_r)

# df_otu_species_tcga_z <- fun_matrix_transfer(df3,method = 'clr_filter');dim(df_otu_species_tcga_z)
# df_otu_species_tcga_s <- fun_matrix_transfer(df4,method = 'clr_filter');dim(df_otu_species_tcga_s)  # otu*sample
...
function_optimal <- function(df1, df2, dist=c("combined", "bray", "jaccard",'euclidean')){
  if (dist=="combined"){
    ...
    # dist_species_tcga_z <- function_dist(df3, dist = "combined")
    # dist_species_tcga_s <- function_dist(df4, dist = "combined")
  }else if (dist=="bray"){
    dist_genus_tcga_z <- function_dist(df1, dist = "bray")
    dist_genus_tcga_s <- function_dist(df2, dist = "bray")
    # dist_species_tcga_z <- function_dist(df3, dist = "bray")
    # dist_species_tcga_s <- function_dist(df4, dist = "bray")
  }else if (dist=="jaccard"){
    dist_genus_tcga_z <- function_dist(df1, dist = "jaccard")
    dist_genus_tcga_s <- function_dist(df2, dist = "jaccard")
    # dist_species_tcga_z <- function_dist(df3, dist = "jaccard")
    # dist_species_tcga_s <- function_dist(df4, dist = "jaccard")
  }else{
    dist_genus_tcga_z <- function_dist(df1, dist = "euclidean")
    dist_genus_tcga_s <- function_dist(df2, dist = "euclidean")
    # dist_species_tcga_z <- function_dist(df3, dist = "euclidean")
    # dist_species_tcga_s <- function_dist(df4, dist = "euclidean")
  }
  ...
  # df_ass_species_tcga_z <- function_assess(dist_species_tcga_z, "TCGA_Z_Species")
  # df_ass_species_tcga_s <- function_assess(dist_species_tcga_s, "TCGA_S_Species")
...
# function_optimal <- function(df1, df2, df3,dist=c("combined", "bray", "jaccard",'euclidean')){
#   if (dist=="combined"){
#     dist_genus_tcga_z <- function_dist(df1, dist = "combined")
#     dist_genus_tcga_s <- function_dist(df2, dist = "combined")
#     dist_genus_tcga_r <- function_dist(df3, dist = "combined")
#     
#     # dist_species_tcga_z <- function_dist(df3, dist = "combined")
#     # dist_species_tcga_s <- function_dist(df4, dist = "combined")
#   }else if (dist=="bray"){
#     dist_genus_tcga_z <- function_dist(df1, dist = "bray")
#     dist_genus_tcga_s <- function_dist(df2, dist = "bray")
#     dist_genus_tcga_r <- function_dist(df3, dist = "bray")
#     
#     # dist_species_tcga_z <- function_dist(df3, dist = "bray")
#     # dist_species_tcga_s <- function_dist(df4, dist = "bray")
#   }else if (dist=="jaccard"){
#     dist_genus_tcga_z <- function_dist(df1, dist = "jaccard")
#     dist_genus_tcga_s <- function_dist(df2, dist = "jaccard")
#     dist_genus_tcga_r <- function_dist(df3, dist = "jaccard")
#     
#     # dist_species_tcga_z <- function_dist(df3, dist = "jaccard")
#     # dist_species_tcga_s <- function_dist(df4, dist = "jaccard")
#   }else{
#     dist_genus_tcga_z <- function_dist(df1, dist = "euclidean")
#     dist_genus_tcga_s <- function_dist(df2, dist = "euclidean")
#     dist_genus_tcga_r <- function_dist(df3, dist = "euclidean")
#     
#     # dist_species_tcga_z <- function_dist(df3, dist = "euclidean")
#     # dist_species_tcga_s <- function_dist(df4, dist = "euclidean")
#   }
...
#   df_ass_genus_tcga_z <- function_assess(dist_genus_tcga_z, "TCGA_Z_Genus")
#   df_ass_genus_tcga_s <- function_assess(dist_genus_tcga_s, "TCGA_S_Genus")
#   df_ass_genus_tcga_r <- function_assess(dist_genus_tcga_r, "TCGA_R_Genus")
#   
#   # df_ass_species_tcga_z <- function_assess(dist_species_tcga_z, "TCGA_Z_Species")
#   # df_ass_species_tcga_s <- function_assess(dist_species_tcga_s, "TCGA_S_Species")
#   
#   df_ass_bind <- rbind(df_ass_genus_tcga_z, df_ass_genus_tcga_s,df_ass_genus_tcga_r)
#   df_ass_bind_melt <- reshape2::melt(df_ass_bind, id=c("ncluster", "class"))
#   df_ass_bind_melt$variable <- as.character(df_ass_bind_melt$variable)
#   df_ass_bind_melt$variable[df_ass_bind_melt$variable=="silinfo"] <- "Average silhouette width"
#   df_ass_bind_melt$variable[df_ass_bind_melt$variable=="predictionS"] <- "Prediction Strength"
...
### 2. Check association between cluster and survival
...
for (x in c('euclidean')){
  ...
  for (j in c('OS','PFS')) {
    ...
    # list_genus_tcga_r <- function_sur_pam(df_otu_genus_tcga_r, dist_s = x,endpoint = j)
    
    # list_species_tcga_z <- function_sur_pam(df_otu_species_tcga_z, dist = x,endpoint = j)
    # list_species_tcga_s <- function_sur_pam(df_otu_species_tcga_s, dist = x,endpoint = j)
    ...
    # p3 <- list_genus_tcga_r[[2]] + ggtitle(paste0(j,'_Genus_tcga_r'))
    
    # p3 <- list_species_tcga_z[[2]] + ggtitle(paste0(j,'_Species_tcga_z'))
    # p4 <- list_species_tcga_s[[2]] + ggtitle(paste0(j,'_Species_tcga_s'))
    ...
    # df_genus_tcga_r <- list_genus_tcga_r[[1]] %>% dplyr::select(c("PATIENT_ID", "Cluster_PAM")) %>% setNames(c("PATIENT_ID", paste("genus_tcga_r_",x,sep = "")))
    
    # df_species_tcga_z <- list_species_tcga_z[[1]] %>% dplyr::select(c("PATIENT_ID", "Cluster_PAM")) %>% setNames(c("PATIENT_ID", paste("species_tcga_z_",x,sep = "")))
    # df_species_tcga_s <- list_species_tcga_s[[1]] %>% dplyr::select(c("PATIENT_ID", "Cluster_PAM")) %>% setNames(c("PATIENT_ID", paste("species_tcga_s_",x,sep = "")))
...
### 3. Visualize the clusters based on overall microbiota

# df_cluster <- df_cluster_all %>% 
#   dplyr::select(c('PATIENT_ID','genus_tcga_s_euclidean','genus_tcga_z_euclidean')) %>% 
#   mutate_all(~ gsub('^1$', 'MS1', .)) %>%   #'^1$':表示匹配完整字符串才替换
#   mutate_all(~ gsub('^2$', 'MS2', .))
# 
# df_cluster_old <- read.table(file = './source_data/ACC_unspervised_clustering.txt',sep = '\t',header = TRUE) %>% 
#   dplyr::select(-id) %>% 
#   mutate_all(~ gsub('^1$', 'MS1', .)) %>% 
#   mutate_all(~ gsub('^2$', 'MS2', .))
# 
# df_cluster_show <- merge(df_cluster,df_cluster_old,by='PATIENT_ID',all.x=TRUE) %>% 
#   tibble::column_to_rownames('PATIENT_ID') %>% 
#   arrange(genus_tcga_s_euclidean) %>% 
#   as.matrix() %>% 
#   t()
# 
# plot_cluster <- ComplexHeatmap::Heatmap(as.matrix(df_cluster_show),
#                                         width = ncol(df_cluster_show)*unit(2, "mm"),
#                                         height = nrow(df_cluster_show)*unit(5, "mm"),
#                                         col = c("#0072B5CC","#E18727CC",'grey'),
#                                         heatmap_legend_param = list(title="Clusters",at=c('MS1','MS2'),labels=c("MS1","MS2")),
...
#                                         column_names_rot = 90)
# 
# # pdf(file = "./clustering/whole/pam/Clustering-comparison.pdf",height = 5,width = 10)
# plot_cluster
# # dev.off()


### 4. Clusters_diversity
...
function_alpha <- function(in_matrix,cluster_col, title){
  ...
  #     select_if(~ sum(.) > 0) %>%
  #     as.matrix()
  #   otu_rarefied <- vegan::rrarefy(in_matrix, sample = min(rowSums(in_matrix)))
  #   otu_matrix <- otu_rarefied
  # }
...
x['g__Anaerococcus',]
z <- data.frame(count=as.numeric(x['g__Anaerococcus',]),
                cpm=as.numeric(x2['g__Anaerococcus',]),
                PATIENT_ID=colnames(x)) %>% 
  merge(.,y,by='PATIENT_ID') %>% 
  dplyr::select(c('PATIENT_ID','cpm','count','OS_Status','OS_Months')) %>% 
  dplyr::mutate(count_group=ifelse(count>0,'high','low')) %>% 
  dplyr::mutate(cpm_group=ifelse(cpm>median(cpm),'high','low'))
write.csv(z,file = './ana.csv')
