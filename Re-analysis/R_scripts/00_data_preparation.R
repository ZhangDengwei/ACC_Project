...
if(TRUE){
  ...
  otu_tcga_z_species_count <- read.csv(file = './source_data/ACC_table_gz/ACC_otutable_bacteria_species.csv',header  = T,check.names = FALSE,row.names = 1) %>% 
    ...
    set_rownames(gsub("[(.*)]", "", rownames(.))) %>%  #去除圆括号
    set_rownames(gsub("\\[|\\]", "", rownames(.))) %>%  #去除方括号
  ...
  # # TCGA_R
  # otu_tcga_r_species_count <- read.xlsx("./source_data/TCGA_R/tcga.oncogene-acc.rs210_species.xlsx", sheet = 1, check.names = FALSE) %>% 
  ...
  #   .[,clinic_tcga$PATIENT_ID]
  # 
  # otu_tcga_r_genus_count <- read.xlsx("./source_data/TCGA_R/tcga.oncogene-acc.rs210_genus.xlsx", sheet = 1, check.names = FALSE)%>% 
  #   aggregate(. ~ Genus,., FUN = mean) %>% 
...
if(TRUE){
  ...
  # ## 3. TCGA_R 
  # otu_tcga_r_species_cpm <- otu_tcga_r_species_count
  # reads_tcga <- read.csv(file = './source_data/CancerLetters_Zhang/ACC.reads.stat.csv')%>% 
  #   dplyr::rename('SAMPLE'='id') %>% 
  #   dplyr::mutate(SAMPLE=str_sub(SAMPLE,1,12)) %>% 
  #   .[match(colnames(otu_tcga_r_species_cpm),.$SAMPLE),]
  # for (i in 1:ncol(otu_tcga_r_species_cpm)) {
  #   otu_tcga_r_species_cpm[,i]<-otu_tcga_r_species_cpm[,i]*10^6/as.numeric(reads_tcga$raw_reads)[i]
  # }
  # 
  # otu_tcga_r_genus_cpm <- otu_tcga_r_genus_count
  # reads_tcga <- read.csv(file = './source_data/CancerLetters_Zhang/ACC.reads.stat.csv')%>% 
  #   dplyr::rename('SAMPLE'='id') %>% 
  #   dplyr::mutate(SAMPLE=str_sub(SAMPLE,1,12)) %>% 
  #   .[match(colnames(otu_tcga_r_genus_cpm),.$SAMPLE),]
  # for (i in 1:ncol(otu_tcga_r_genus_cpm)) {
  #   otu_tcga_r_genus_cpm[,i]<-otu_tcga_r_genus_cpm[,i]*10^6/as.numeric(reads_tcga$raw_reads)[i]
  # }
