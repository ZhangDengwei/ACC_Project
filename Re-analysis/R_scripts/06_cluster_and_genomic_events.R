...
### mutation and cnv landsccape between MS1 and MS2
if(TRUE){
  ...
  #          sortByMutation = F,
  #          fontSize = 0.5,
  #          writeMatrix = F,  #export as ‘oncomatrix.txt’ file
  #          clinicalFeatures = c('MS'),
  #          logColBar=T,
  #          sortByAnnotation = F)
...
fun_oncoplot_heatmap <- function(mat,clinic){
  ...
  # column
  clinic <- clinic[order(clinic$MS),]  # clinic按照kmeans排序
  mat <- mat[,clinic$PATIENT_ID] #列跟clinic相同顺序
  
  colomn_group <- c(rep('MS1',42),rep('MS2',35)) #列分组信息
...
### TMB and FGA between ms1 and ms2
