...
df_meta_acc_tcga <- read.csv("./00_data_preparation/metadata.csv",na.strings = "") %>% 
  filter(COHORT=='TCGA') 
df_meta_acc_hs <- read.csv("./00_data_preparation/metadata.csv",na.strings = "") %>% 
  filter(COHORT=='HUASHAN') 
...
df_count_3 <- read.xlsx('./00_data_preparation/otu_genus_count_table.xlsx',sheet = 3,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric))
...
df_cpm_3 <- read.xlsx('./00_data_preparation/otu_genus_cpm_table.xlsx',sheet = 3,rowNames = TRUE) %>% mutate(across(where(is.character), as.numeric)) %>%log1p()
...
df_com_genus_hs <- fun_matrix_transfer(df_count_3,method = 'com');dim(df_com_genus_hs)
...
df_cpm_genus_hs <- df_cpm_3 %>% t() %>% .[rownames(df_com_genus_hs),colnames(df_com_genus_hs)]


common_otu <- Reduce(intersect,list(colnames(df_com_genus_tcga_z),colnames(df_com_genus_tcga_s)))

### 1. Examine the confounders impacting microbial community
...
function_Maaslin2 <- function(input_data, df_clinical_cluster,group, output_dir){
  ...
  fit_data = Maaslin2(
    input_data = input_data,   # 微生物丰度矩阵
    input_metadata = df_clinical_cluster,  # 样本元数据
    min_prevalence = 0,
    # normalization="NONE",
    # transform="NONE",
    analysis_method = "LM",
    ...
    fixed_effects = group,
    # random_effects = c("SEX", 'RACE', "AGE", "STAGE"),
    cores = 6)
...
fun_diff_visual <- function(result_maaslin){
  ...
  p<- ggplot(data = df_features, aes(x=coef, y=log.pval, label = ifelse(common=='Yes', feature, ""),fill=class))+
    ...
    geom_text_repel(aes(color=class),size = 4, box.padding = 0.5, max.overlaps = 25) +  # 避免重叠
...
# feature prevalance
fun_prevalance_barplot <- function(otu_list,df1,df2,df3){
  
  otu_list_1 <- intersect(otu_list,colnames(df1))
  microbes_1 <- as.data.frame(t(df1[,otu_list_1])) %>% 
    dplyr::mutate(Mean_prevalance =rowMeans(as.matrix(.))) %>% 
    dplyr::mutate(Proportion=rowMeans(. > 0, na.rm = TRUE))
  matrix_1 <- data.frame(Genus=rownames(microbes_1),
                       Mean_prevalance=microbes_1$Mean_prevalance,
                       Proportion=microbes_1$Proportion) %>% 
    dplyr::mutate(cohort_name='TCGA_S') 
  
  otu_list_2 <- intersect(otu_list,colnames(df2))
  microbes_2 <- as.data.frame(t(df2[,otu_list_2])) %>% 
    dplyr::mutate(Mean_prevalance =rowMeans(as.matrix(.))) %>% 
    dplyr::mutate(Proportion=rowMeans(. > 0, na.rm = TRUE))
  matrix_2 <- data.frame(Genus=rownames(microbes_2),
                       Mean_prevalance=microbes_2$Mean_prevalance,
                       Proportion=microbes_2$Proportion) %>% 
    dplyr::mutate(cohort_name='TCGA_Z') 
  
  otu_list_3 <- intersect(otu_list,colnames(df3))
  microbes_3 <- as.data.frame(t(df3[,otu_list_3])) %>% 
    dplyr::mutate(Mean_prevalance =rowMeans(as.matrix(.))) %>% 
    dplyr::mutate(Proportion=rowMeans(. > 0, na.rm = TRUE)) 
  matrix_3 <- data.frame(Genus=rownames(microbes_3),
                       Mean_prevalance=microbes_3$Mean_prevalance,
                       Proportion=microbes_3$Proportion) %>% 
    dplyr::mutate(cohort_name='HUASHAN') 
  
  matrix_all <- Reduce(rbind,list(matrix_1,matrix_2,matrix_3))
  matrix_all$cohort_name <- factor(matrix_all$cohort_name,levels = c('TCGA_S','TCGA_Z','HUASHAN'))

  
  p1 <- ggplot(matrix_all, aes(y = Genus, x = Mean_prevalance)) +
    geom_bar(fill="#DF8F44FF",stat = "identity") + # Use a similar dark teal color
    geom_text(aes(label = sprintf("%.1f%%", Mean_prevalance * 100)),  
              hjust = -0.1,  # 将标签放在条形右侧
              vjust = 0.5,
              size = 2,  
              color='black')+
    facet_wrap(~ cohort_name, ncol = 3) +
    labs(title = "",y = "",x = "Mean Prevalance") +
    theme_bw()+
    theme(panel.border = element_rect(colour = "black",size = 1),
          axis.text.x = element_text(color ='black'),
          axis.text.y = element_text(color ='black'))+
    scale_x_continuous(expand = expansion(mult = c(0, 0.3)))  # 右侧留更多空间
  
  p2 <- ggplot(matrix_all, aes(y = Genus, x = Proportion)) +
    geom_bar(fill="#DF8F44FF",stat = "identity") + # Use a similar dark teal color
    geom_text(aes(label = sprintf("%.1f%%", Proportion * 100)),  
              ...
              color='black')+
    facet_wrap(~ cohort_name, ncol = 3) +
    labs(title = "",y = "",x = "Present at % samples ") +
    theme_bw()+
    theme(panel.border = element_rect(colour = "black",size = 1),
          axis.text.x = element_text(color ='black'),
          axis.text.y = element_text(color ='black'))+
...
# pdf(file = paste0('./microbial_feature/prevalance_feature.pdf'),height = 3,width = 8)
print(fun_prevalance_barplot(core_features,df_com_genus_tcga_s,df_com_genus_tcga_z,df_com_genus_hs)[[1]])
# dev.off()

# pdf(file = paste0('./microbial_feature/Proporation_feature.pdf'),height = 3,width = 8)
print(fun_prevalance_barplot(core_features,df_com_genus_tcga_s,df_com_genus_tcga_z,df_com_genus_hs)[[2]])
# dev.off()


### 5. Stratify patients based on the abundance of significant microbes
core_features
function_ex_sur <- function(in_matrix,source=c('TCGA')){
  df_cox_OS <- data.frame()
  df_cox_PFS <- data.frame()
  list_OS <- list()
  list_PFS <- list()
  df_meta_acc <- df_meta_acc_tcga
  
  
  features <- intersect(core_features,colnames(in_matrix))
  for (i in 1:length(features)){
    genus <- features[i]
    if (genus %in% colnames(in_matrix)){
      df_s <- in_matrix[,features[i], drop=FALSE] %>% as.data.frame()
      ...
      if(all(df_s[,genus]== 0) ){
        ...
        df_meta_ACC_m <- merge(df_meta_acc[, c("PATIENT_ID", "OS_Months", "OS_Status",'PFS_Months', 'PFS_Status')], df_s, by="PATIENT_ID")
        ...
        if (inherits(try_result, "try-error")) {
          message("Using median as fallback cutpoint")
          fit1 <- survfit(Surv(OS_Months, OS_Status) ~ abundance, data = df_meta_ACC_m)
          p1 <- ggsurvplot(fit1,
                           data = df_meta_ACC_m,
                           conf.int = TRUE,
                           pval = TRUE,
          ...
          res.cut1 <- surv_cutpoint(df_meta_ACC_m, #数据集
                                    time = "OS_Months",
                                    event = "OS_Status",
                                    minprop = 0.1,
                                    variables = 'taxa') #需要计算的数据列名
        ...
        # cox analysis
        res.cox1 <- coxph(Surv(OS_Months, OS_Status) ~ taxa_abun, data = df_meta_ACC_m) #按照丰度做cox
        # res.cox1 <- coxph(Surv(OS_Months, OS_Status) ~ taxa, data = dat1) #按照最佳分组做cox
...
p_cpm_tcga_s<- function_ex_sur(df_cpm_genus_tcga_s,'TCGA')
p_cpm_tcga_z<- function_ex_sur(df_cpm_genus_tcga_z,'TCGA')

## KM plot

# OS

# pdf(file = "./microbial_feature/Biomarker_survial_examination_os_cpm_tcga_s.pdf",height = 4,width = 5)
p_cpm_tcga_s
# dev.off()

# pdf(file = "./microbial_feature/Biomarker_survial_examination_os_cpm_tcga_z.pdf",height = 4,width = 5)
p_cpm_tcga_z
# dev.off()
...
dd_1 <- rms::datadist(data_1)   # 设置数据分布
...
dd_2 <- rms::datadist(data_2)   # 设置数据分布
