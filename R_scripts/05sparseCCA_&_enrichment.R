...
microbes <- acc_micro_matrix[,ms2_patient]
microbes <- t(microbes)
dim(microbes)
# 43 144
...
genes <- acc_exp[,clinic$PATIENT_ID]
genes <- t(genes)
FUN =  function(xxx) {
  (sum(xxx==0)/length(xxx))<0.5 }
genes <- genes[,apply(genes, 2, FUN)]
genes.t <- t(genes)
genes <- filter_genes(genes,2) #filter low expression genes
genes <- genes[ms2_patient,]
dim(genes)
# 43 14267
...
dirname <- paste0("./data/sparse_cca_analysis/ACC/output_sparseCCA_V2/grid_search/gene_taxa_components/sig_gene_taxa_components_",bestpenaltyX,"_", bestpenaltyY,"_padj/")
...
# save_CCA_components(CCA.out[[1]],sig,dirname)
...
input_dir <- paste0("./data/sparse_cca_analysis/ACC/output_sparseCCA_V2/grid_search/gene_taxa_components/",input_enrichment)
...
output_dir <- paste0("./data/sparse_cca_analysis/ACC/output_sparseCCA_V2/grid_search/enrichment_sparseCCA_msigDB/",msigdb_collection_name,"/per_component_kegg/before_MHT/25_300_5_X/")
