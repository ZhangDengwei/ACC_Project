---
title: "10.Host_Microbiome_Analysis"
...
exp <- read.table(file = './03transcriptomic_analysis/output_acc_filtered_mrna_exp_log_transformed.tsv',header = T,row.names = 1,sep = '\t',check.names = F) %>% t()
...
## Sparse CCA
```{r,eval=FALSE}
enrichment_sparseCCA_msigDB_result <- list()
for (k in 1:4) {
  # data preparation
  ...
  enrichment_sparseCCA_msigDB_result[[k]] <- enrichment_case_no_dups
}
```

```{r}
enrichment_sparseCCA_msigDB_result_NR <- read.table(file = './04host_microbiome_analysis/enrichment_sparseCCA_msigDB/enrichment_kegg_Without_combined.txt',sep = '\t',header = T,row.names = 1)
knitr::kable(enrichment_sparseCCA_msigDB_result_NR)
enrichment_sparseCCA_msigDB_result_LR <- read.table(file = './04host_microbiome_analysis/enrichment_sparseCCA_msigDB/enrichment_kegg_Likely_combined.txt',sep = '\t',header = T,row.names = 1)
knitr::kable(enrichment_sparseCCA_msigDB_result_LR)
enrichment_sparseCCA_msigDB_result_CR <- read.table(file = './04host_microbiome_analysis/enrichment_sparseCCA_msigDB/enrichment_kegg_PC_combined.txt',sep = '\t',header = T,row.names = 1)
knitr::kable(enrichment_sparseCCA_msigDB_result_CR)
enrichment_sparseCCA_msigDB_result_PR <- read.table(file = './04host_microbiome_analysis/enrichment_sparseCCA_msigDB/enrichment_kegg_Putative_combined.txt',sep = '\t',header = T,row.names = 1)
knitr::kable(enrichment_sparseCCA_msigDB_result_PR)
```

## Visualization of specific pathway
```{r}
#cell cycle
knitr::include_graphics("./04host_microbiome_analysis/cell cycle_00.png")
#p53 signaling pathway
knitr::include_graphics("./04host_microbiome_analysis/p53_00.png")
```
